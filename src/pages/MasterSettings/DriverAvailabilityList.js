...
export default function DriverAvailabilityList() {
  const accessRights = useSelector((state) => state.auth.accessRights);
  const userRole = accessRights && accessRights.rights_role;
  const { sidebar, recordsPerPage, useDebounce } = useContext(AppContext);
  const [driverAvailableList, setDriverAvailableList] = useState([]);
  ...
  function getAvailbilitylist(currentPage) {
    let newRequestBody = JSON.stringify({
      driver_name: FiltertData,
      trip_driver_id: "",
      page: currentPage,
      page_limit: recordsPerPage,
      format: OptionDownload,
    });
    simplePostCall(ApiConfig.DRIVER_AVALIVALITY_LIST, newRequestBody)
      .then((data) => {
        console.log(data);
        if (data.result) {
          ...
          setpdfData(firstKey);
          if (!firstKey) {
            setDriverAvailableList(data.data );
          ...
          if (currentPage === 1) {
            ...
            setDriverAvailableList([...driverAvailableList, ...data.data]);
          }
        } else {
          setDriverAvailableList();
          setlast_page(false);
        }
      })
      .catch((err) => {
    ...
    >
      <div id="cx-wrapper" className="Vehicle_inspition">
        ...
        <div className="main-master-wrapper">
            ...
            <div className="VA_table">
              <table className="table tableAdmin">
                ...
                <tbody className="tableBody">
                  {driverAvailableList &&
                    driverAvailableList?.map((data, index) => {
                      console.log("driverAvailableList", driverAvailableList);
                      return (
                        <tr key={index + 1}>
                          <td>{index + 1}</td>
          ...
          {driverAvailableList?.length > 0 && (
            <Pagenation
              length={driverAvailableList?.length}
              total={driverAvailableList.length}
            />
          )}
        </div>
      </div>
