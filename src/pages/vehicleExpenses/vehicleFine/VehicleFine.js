...
const VehicleFine = () => {
  ...
  const { sidebar, setSidebar, Dark, setDark, /* loading, setLoading , */recordsPerPage, setOptionDynamicDownload} =
  ...
  //   setPage(page + 1)
  //   // getSparepart(page + 1)
  //   reportViewList( currentDate.toDayDate,yearStartDate, yearEndDate, firstDayOfMonth, endDayOfMonth, NextDateShow.toNextDate, optionData, weekStartDate, weekEndDate, page + 1 );
  //   }
  // }
...
const reportViewList = async (
  ...
  if (selelctedDrop === "month") {
    requestData = {
      ...
      fine_reason: FilterReason,
      fine_date: latestDate(firstmonth, "yyyy-MM-dd"),
    ...
    requestData = {
      ...
      vehicle_name: FilterName?.vehicle_name ? FilterName?.vehicle_name?.toLowerCase() : '', 
      fine_reason: FilterReason,
      fine_date: latestDate(yearStart, "yyyy-MM-dd"),
    ...
    requestData = {
      ...
      vehicle_name: FilterName?.vehicle_name ? FilterName?.vehicle_name?.toLowerCase() : '', 
      fine_reason: FilterReason,
      fine_date: latestDate(weekStart, "yyyy-MM-dd"),
    ...
    requestData = {
      ...
      vehicle_name: FilterName?.vehicle_name ? FilterName?.vehicle_name?.toLowerCase() : '', 
      fine_reason: FilterReason,
      fine_date:latestDate(nextDate, "yyyy-MM-dd"),
  ...
  if (requestData) {
    ...
    OptionDownload && set_spinner_loading(true);
      const res = await simplePostCallPromise(ApiConfig.VEHICLE_FINE_FILTER, JSON.stringify(requestData));
      if (res?.result) {
        setLoading(false);
        setOptionDynamicDownload("");
...
useEffect(()=>{
  ...
  reportViewList(
    ...
    FilterAmmount,
     FilterReason,  yearStartDate, yearEndDate,trigger
]);
useEffect(()=>{
  setPage(1);
  setTrigger(true);
},[FilterName?.vehicle_name, FilterReason,FilterAmmount, optionData
/* weekEndDate,
weekStartDate,
// currentDate,
NextDateShow,
...
FilterName?.vehicle_name,
FilterAmmount,
 FilterReason,  yearStartDate, yearEndDate, */])
