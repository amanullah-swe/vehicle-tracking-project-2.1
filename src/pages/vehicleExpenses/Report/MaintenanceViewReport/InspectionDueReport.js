 ...
 /*  const getdistanceunit = localStorage.customer_distance_unit
  ...
  /* const reportViewList = () => {
    ...
    if (requestData) {
      ...
      setOptionDownload("");
      simplePostCall(
        ApiConfig.GET_VEHICLE_INSPECTION_EXPIRY_REPORT ,
  ...
  const reportViewList = (
    ...
    if (requestData) {
      ...
      setOptionDownload("");
      simplePostCall(
        ApiConfig.GET_VEHICLE_INSPECTION_EXPIRY_REPORT /* + LinkReport */,
        JSON.stringify(requestData)
      )
        .then((res) => {
          if (res?.result) {
...
export default InspectionDueReport;
