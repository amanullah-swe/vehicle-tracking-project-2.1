...
const { Option } = Select;
const MaintenanceDuesSoonReport = () => {
 ...
 /*  const getdistanceunit = localStorage.customer_distance_unit
  ...
  //     setOptionDownload("");
  //     simplePostCall(
  //       ApiConfig.MANTENANCE_DUES_SOON_REPORT ,
  ...
  const reportViewList = (
    ...
    if (requestData) {
      ...
      setOptionDownload("");
      simplePostCall(
        ApiConfig.MANTENANCE_DUES_SOON_REPORT /* + LinkReport */,
        JSON.stringify(requestData)
      )
        .then((res) => {
          if (res?.result) {
...
export default MaintenanceDuesSoonReport;
