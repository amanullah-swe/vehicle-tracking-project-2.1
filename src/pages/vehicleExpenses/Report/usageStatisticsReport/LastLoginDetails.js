...
const Component1 = () => {
    ...
    const reportViewList = (
        ...
        if (requestData) {
            ...
            setOptionDownload("");
            simplePostCall(ApiConfig.LAST_LOGIN_DETAILS, JSON.stringify(requestData)).then((res) => {
                if (res.result) {
                    ...
                    setpdfData(firstKey);
                    if (!firstKey) {
                        setOrderListData(res.report_data);
                    }
                        ...
                        <table className="table tableAdmin">
                            <thead className="tableHead">
                                <tr>
                                    <th>{t("Sr.no")}</th>
                                    <th>{t("Name")}</th>
                                    <th className="w-2">{t("User Role")}</th>
                                    <th>{t("Platform")}</th>
                                    <th>{t("Last Login Date")}</th>
                                                ...
                                                return (
                                                    <>
                                                        <tr>
                                                            ...
                                                            <td>
                                                                {itemlist.user_access_mode
                                                                    ? itemlist.user_access_mode
                                                                    : ""}
                                                            </td>
                                                            <td>
                                                                {itemlist.user_last_login
                                                                    ? itemlist.user_last_login
                                                                    : ""}
                                                            </td>
