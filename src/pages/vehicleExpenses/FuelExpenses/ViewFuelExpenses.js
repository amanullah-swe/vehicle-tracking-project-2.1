...
const ViewFuelExpenses = () => {
  ...
  const handleClickImage = (image_path) => {
    set_selected_image(image_path);
    setShowModal(true);
    ...
    <>
      ...
      >
        {loading ? (
          ...
          <>
            <div id='cx-wrapper' className='ViewFuel_Expenses'>
              <div className='main-master-wrapper'>
                <div className='headingDetails'>
                  <div className='headingTxt'>
                    <p className='heading'>{t('Fuel Expenses Details')}</p>
                  </div>
                  {userRole === 'customer' ||
                    (accessRights &&
                ...
                <div className='DetailsSec'>
                  <div className='row detailsRow'>
                    ...
                    <div className='col-md-3 ViewCol3 form_input_main'>
                      <label className='head'>{t('Current Odometer')}</label>
                      <p className='Value'>
                        {AccesoryDetails.vehicle_fuel_current_odometer}
                      </p>
                    </div>
                    <div className='col-md-3 ViewCol3 form_input_main'>
                      <label className='head'>{t('Quantity')}</label>
                      <p className='Value'>{AccesoryDetails.vehicle_fuel_quantity}</p>
                    </div>
                    <div className='col-md-3 ViewCol3 form_input_main'>
                      <label className='head'>{t('Amount')}</label>
                      <p className='Value'>{AccesoryDetails.vehicle_fuel_amount}</p>
                    </div>
                    <div className='col-md-3 ViewCol3 form_input_main'>
                      <label className='head'> {t('Date')}</label>
                      <p className='Value'>
                        {DateDDMMYYYY(AccesoryDetails.vehicle_fuel_date)}
                      </p>
                    </div>
                      ...
                      <div className='invoiceBox d-flex justify-content-start align-items-center flex-wrap'>
                        ...
                        {AccesoryDetails?.invoices?.map((invoice, index) => (
                          <div className='saveFile mx-2 my-2'>
                            <Link to="#" onClick={() => /* setShowModal(true) */ handleClickImage(invoice?.expense_file_path)} key={index}>
                              {isImage(invoice.incidentOG) ? (
                                <>
                                  ...
                                  >
                                    <Modal.Body>
                                      {/* <img src={AccesoryDetails.fuel_invoice_no} className='invoiceBigImg' alt='' /> */}
                                      {selected_image &&
                                        <img src={selected_image} className='invoiceBigImg' alt='' />
                                      }
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>{t('Delete Fuel Expenses Invoice')}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {t('Are you sure you want to Delete this Fuel Expenses Invoice')} ?
        </Modal.Body>
        <Modal.Footer className='pop-up-modal-footer'>
          <div class='btn-wrapper'>
            <button className='cx-btn-1' onClick={() => setInvoiceModal(false)}>
              {t('Cancel')}
            </button>
...
export default ViewFuelExpenses
