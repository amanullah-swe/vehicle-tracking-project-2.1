...
const TeamComposeMessage = () => {
  const { Option } = Select;

  const navigate = useNavigate();
  const { sidebar, setSidebar, Dark, customerData,socket } = useContext(AppContext);
  ...
  const AddCommpose = (e) => {
    ...
    socket.emit("create-channel", {
      ...
      "channelName":AddAccessry.subject,
      "memberIds":userName
  
      }
      
    );
    navigate("/TeamMessages");
  };
  ...
  return (
    <main className={sidebar ? "taskMain " : "cx-active taskMain"} id="cx-main">
      <div id="cx-wrapper" className="TeamComposeMessage">
...
export default TeamComposeMessage;
